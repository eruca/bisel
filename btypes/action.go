...
func handlerFunc(tabler Tabler, size int, orderby string, pt ParamType, jwtSession Defaulter, handlers ...Action) ContextConfig {
	return func(c *Context) ParamType {
		// 重置参数
		tabler = tabler.New()

		// 将客户端发送过来的Payload => ParamsContext
		pc := ParamsContextFromJSON(tabler, size, orderby, pt, c.Request.Payload)
...
func QueryHandler(tabler Tabler, size int, orderby string, handlers ...Action) ContextConfig {
	return handlerFunc(tabler, size, orderby, ParamQuery, nil, handlers...)
}
